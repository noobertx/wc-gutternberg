(function ($) {
    'use strict';
    if( typeof wprigInteraction === 'undefined')
        return
    /*
     * ====================
     * INTERACTION MATH
    ...
    function initiateTiltAnimation( windowWidth ){
      if( typeof wprigInteraction['mouse_movement'] !== 'undefined' ){
          let mouseMovementList = wprigInteraction['mouse_movement']
          mouseMovementList.map( tiltOpt => { 
              let $addonElem = $(`.wprig-block-${tiltOpt.blockId}`)
              ...
              let element =  document.getElementsByClassName(`wprig-block-${tiltOpt.blockId}`)
              if( element.length > 0 ){
    ...
    if(typeof wprigInteraction['while_scroll_view'] !== 'undefined'){
    ...
    //  }
      ...
      let scrollActionList = wprigInteraction['while_scroll_view']         
      let $window = $(window)
      ...
      $(document).ready(function(){
        scrollActionList.map( animationProperty => {
          ...
          let $addonElem = $(`.wprig-block-${animationProperty.blockId}`)
